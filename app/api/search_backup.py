...
async def search_analysis_results(request: SearchRequest):
    ...
    try:
        ...
        JOIN analysis_jobs aj ON ar.job_id = aj.job_id
        ...
        # 🔍 검색 조건 추가
        if request.uid:
            conditions.append("ar.uid LIKE ?")
            params.append(f"%{request.uid}%")
        
        if request.query:
            # 통합 검색 - UID나 결과 데이터에서 검색
            conditions.append("(ar.uid LIKE ? OR ar.result_data LIKE ?)")
            params.extend([f"%{request.query}%", f"%{request.query}%"])
        
        if request.department:
            conditions.append("ar.result_data LIKE ?")
        ...
        if request.score_min is not None:
            conditions.append("CAST(json_extract(ar.result_data, '$.AIRISS_v4_종합점수') AS REAL) >= ?")
        ...
        if request.date_from:
            conditions.append("aj.created_at >= ?")
        ...
        # 정렬 추가
        order_mapping = {
            "score": "CAST(json_extract(ar.result_data, '$.AIRISS_v4_종합점수') AS REAL)",
            "date": "aj.created_at",
            "name": "ar.uid",
            "grade": "json_extract(ar.result_data, '$.OK등급')"
        }
        ...
        logger.info(f"📝 실행 쿼리: {base_query}")
        ...
        async with (await db_service.get_connection()) as conn:
            ...
            # 전체 개수 조회 (페이징용) - 같은 커넥션 사용
            count_query = base_query.split("ORDER BY")[0].replace(
                "SELECT DISTINCT ar.uid, ar.result_data, aj.created_at as analysis_date, aj.file_id, aj.job_id",
                "SELECT COUNT(DISTINCT ar.uid)"
            )
            
            count_cursor = await conn.execute(count_query, params[:-2] if "LIMIT" in base_query else params)
...
async def get_autocomplete_suggestions(request: AutocompleteRequest):
    ...
    try:
        ...
        if request.field == "uid":
            ...
            # 등급 자동완성
            query = """
            SELECT DISTINCT json_extract(ar.result_data, '$.OK등급') as grade
            FROM analysis_results ar
            WHERE json_extract(ar.result_data, '$.OK등급') LIKE ?
            ORDER BY grade
            LIMIT ?
            """
            params = [f"%{request.query}%", request.limit]
            ...
            # 부서 자동완성 (결과 데이터에서 추출)
            query = """
            SELECT DISTINCT json_extract(ar.result_data, '$.부서') as dept
            FROM analysis_results ar
            WHERE json_extract(ar.result_data, '$.부서') LIKE ?
            AND json_extract(ar.result_data, '$.부서') IS NOT NULL
            ORDER BY dept
            LIMIT ?
            """
            params = [f"%{request.query}%", request.limit]
...
async def get_employee_history(
    ...
    try:
        ...
        FROM analysis_results ar
        JOIN analysis_jobs aj ON ar.job_id = aj.job_id
        WHERE ar.uid = ? AND aj.status = 'completed'
...
async def compare_employees(request: CompareRequest):
    ...
    try:
        ...
        for uid in request.uids:
            query = """
            SELECT ar.result_data, aj.created_at
            FROM analysis_results ar
            JOIN analysis_jobs aj ON ar.job_id = aj.job_id
            WHERE ar.uid = ? AND aj.status = 'completed'
            ORDER BY aj.created_at DESC
            ...
            async with (await db_service.get_connection()) as conn:
                cursor = await conn.execute(query, [uid])
...
async def get_team_summary(
    ...
    try:
        ...
        # 부서 필터
        if department:
            base_query += " AND ar.result_data LIKE ?"
            params.append(f'%"부서":"%{department}%"%')
        
        # 날짜 필터
        if date_from:
            base_query += " AND aj.created_at >= ?"
...
# 🎯 즐겨찾기 관리 (새로 추가)
favorites_storage = {}  # 실제로는 DB에 저장 (user_id -> [uid1, uid2, ...])
...
async def add_favorite(request: FavoriteRequest):
    ...
    try:
        ...
        # 실제로는 DB에 저장하지만, 프로토타입용으로 메모리 사용
        ...
        if include_details:
            ...
            for favorite in user_favorites:
                ...
                try:
                    ...
                    logger.error(f"⚠️ 즐겨찾기 상세 정보 조회 오류 ({uid}): {e}")
                    detailed_favorite = favorite.copy()
...
async def search_health_check():
    ...
    try:
        ...
        return {
            "status": "healthy",
            "service": "AIRISS Search API v4.1 Enhanced",
            "database": db_status,
            "features": [
                "고급 검색", "자동완성", "직원 히스토리", 

...
# 1. File 모델 수정 (metadata -> file_metadata)
...
# 2. Employee 모델도 수정 (metadata -> employee_metadata)
...
# 4. 기존 데이터베이스 파일 삭제 (새로운 스키마 적용)
import os
if os.path.exists('airiss_v4.db'):
    os.remove('airiss_v4.db')
